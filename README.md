...
![equation](https://latex.codecogs.com/svg.image?(\mathbf{\Sigma}_{L1},\mathbf{V}_{L1})=\underset{\mathbf{V} \in \mathbb{S}^{N\times K},\mathbf{\Sigma} \in diag(\mathbb{R}^{K})}argmin||\mathbf{X}^T\mathbf{U}_{L1}-\mathbf{V}_{L1}\mathbf{\Sigma}_{L1}||_{1,1}&space;)
---
